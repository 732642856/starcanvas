...
| 爆款结构拆解 / remix analysis | `real provider integrated` | 打开结构拆解面板，导入结果为 `remix-analysis` 节点供后续参考 | 还不是直接连到完整复刻生产链的主入口，更多是分析资产而非自动生产流程 | `apps/web/src/app/canvas/components/panels/VideoRemixPanel.tsx`, `apps/web/src/app/canvas/hooks/useWorkflowRunner.ts`, `apps/web/src/app/canvas/StarCanvas.tsx` | 当前缺少专门 E2E；依赖导入与节点链路的集成验证 |
...
| 6 | 爆款结构拆解结果继续进入真实生产链（本轮已接通最短链） | `roundtrip verified`（局部） | 结构拆解不再只停在“分析资产”，而是能直接派生出可继续生产的下游对象 | 已达成：`remix-analysis` 节点可一键派生复刻提示词、参考分镜节点、生产队列；剩余观察点是补右键 UI/E2E，把“队列创建 -> 实际跑图/跑视频”连成更长证据链 | `apps/web/src/app/canvas/components/menus/NodeContextMenu.tsx`, `apps/web/src/app/canvas/utils/remixAnalysisArtifacts.ts`, `apps/web/src/app/canvas/StarCanvas.tsx`, `apps/web/src/features/production/useShotPlanningRunQueueStore.ts` | `apps/web/src/app/canvas/utils/remixAnalysisArtifacts.test.ts`；后续需补 `remix-analysis -> 下游节点生成` UI/E2E |
| 7 | 本地图片/媒体到 provider 可访问 URL 的通用桥接层 | `roundtrip verified`（局部） | reverse-prompt 主链已通，但更通用的素材桥接仍是多个点状修补，后续会卡住更多真实工作流 | 本地图片、视频等素材都能稳定进入需要远程 URL 的 provider，不靠单点特判 | `apps/web/src/app/canvas/utils/reversePromptNodeAction.ts`, `apps/web/src/lib/assets/`, `apps/web/src/app/canvas/utils/videoSourceImage.ts` | 现有 `uploaded-image-reverse-prompt.spec.ts` 是基础，后续需扩到更多 provider 消费路径 |
